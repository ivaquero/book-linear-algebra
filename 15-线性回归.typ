...
$
  "SSD"(𝑿) & = sum norm(𝒙^⊤ - μ_(𝑿))^2                \
           & = (𝒙^⊤ 𝑴^⊤)(𝑴 𝒙) = 𝒙^⊤ 𝑴 𝒙 = tr(𝑿_c^⊤ 𝑿)
$

基于上述结果，数据集的方差写作

$ "Var"(𝑿) = 1 / n sum (X_i - 𝔼[𝑿])^2 = 1 / n "SSD" $
...
#definition[
  设$𝑨$是$n$阶 Hermitan 矩阵，$𝒙$是$n$维非零列向量，则称

  $
    R(𝑨,𝒙) = frac(𝒙^H 𝑨 𝒙, 𝒙^H 𝒙)
  $

  为 Rayleigh 商，且$R(𝑨,𝒙) ∈ [λ_(min),λ_(max)]$，其中，$λ$为$𝑨$的特征值。
]

#tip[
  当向量$𝒙$是标准正交基时，即满足$𝒙^H 𝒙 = 1$时，Rayleigh 商退化为$R(𝑨, 𝒙) = 𝒙^H 𝑨 𝒙$，这个形式在频谱聚类和 PCA 中都有出现。
]

犹豫 Hermitan 矩阵可对角化，即有$𝑨 = 𝑽 𝜦 𝑽^⊤$，于是
...
$
  R(𝒙) = frac(𝒚^⊤ 𝜦 𝒚, 𝒚^⊤ 𝒚) = frac(sum λ_i y_i^2, sum y_i^2)
$
...
$
  R(𝑨, 𝑩, 𝒙) = frac(𝒙^H 𝑨 𝒙, 𝒙^H 𝑩 𝒙)
$

其中，$𝒙$为非零向量，$𝑨$为$n$阶 Hermitan 矩阵。当$𝑩$为正定矩阵

$
  R(𝑨, 𝑩, 𝒙) ∈ [λ(𝑩^(-1)𝑨)_(min), λ(𝑩^(-1 / 2)𝑨 𝑨 𝑩^(-1 / 2))_(max)]
$
...
$
  𝑿^⊤ 𝑿 & = mat(
            delim: "[",
            sum_(i = 0)^4 (x_0^((i)))^2, sum_(i = 0)^4 x_0^((i)) x_1^((i)), sum_(i = 0)^4 x_0^((i)) x_2^((i));
            sum_(i = 0)^4 x_1^((i)) x_0^((i)), sum_(i = 0)^4 (x_1^((i)))^2, sum_(i = 0)^4 x_1^((i)) x_2^((i));
            sum_(i = 0)^4 x_2^((i)) x_0^((i)), sum_(i = 0)^4 x_2^((i)) x_1^((i)), sum_(i = 0)^4 (x_2^((i)))^2
          )                                                                       \
