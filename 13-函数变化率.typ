...
#definition[
  ...
  由于此导数形式与其分母部分相同，故称此形式为分母布局（denominator layout）。类似地，若导数为行向量形式，称分子布局（nominator layout）。
]

== 向量对向量
<向量对向量>

#theorem(ctext("标量链式法则"))[
  对$y = f(u), u = g(x)$，有

  $
    pdv(y, x) = pdv(y, u)pdv(u, x)
  $
]

#theorem(ctext("向量链式法则"))[
  $
    ...
    underbrace(pdv(𝒚, 𝒙), (m,n)) = underbrace(pdv(𝒚, 𝒖), (m,k)) underbrace(pdv(𝒖, 𝒙), (k,n))
  $
]

#definition[
  对向量$𝒚_(1×m)$，若其函数$𝒇(𝒚)$为向量，即

  $
    𝒇(𝒚) = mat(delim: "[", f_1(𝒚), f_2(𝒚), ⋯, f_n (𝒚))_(1×n)
  $
  ...
  $
    ...
    mat(
      delim: "[",
      pdv(f_1(𝒚), y_1), , …, pdv(f_n(𝒚), y_1);
      ⋮, ⋱, , ⋮;
      ⋮, , ⋱, ⋮;
      pdv(f_1(𝒚), y_m), …, …, pdv(f_n(𝒚), y_m)
    )_(m×n)
  $
]
