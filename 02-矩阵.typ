...
$ 𝑪_(i j) = (𝑨 𝑩)_(i j) = ∑_(k=1)^n a_(i k) b_(k j) $
...
$
  𝑨:𝑩 = ⟨𝑨, 𝑩⟩_F &= ∑_j∑_j A_(i j)B_(i j) \
...
#definition[
  若对$n$阶单位矩阵$𝑰$行运算就可以得到$n$阶方阵$𝑬$，则称为初等矩阵（elementary matrix）。
]
...
#theorem[
  当且仅当$n$阶方阵$𝑨$与$n$阶单位矩阵$𝑰$行等价时，$n$阶方阵$𝑨$可逆。
]
...
$
  (𝑰 + α β^H)^(-1) = 𝑰 + ∑_(i=1)^n (-1)^i α (β^H α)^(i-1) = 𝑰 - frac(α β^H, 1 + β^H α)
$
